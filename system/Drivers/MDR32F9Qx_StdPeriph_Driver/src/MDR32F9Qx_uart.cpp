						   ...
						   UART_InitTypeDef* UART_InitStruct )
{
	...
	/* Check the parameters */
	assert_param(IS_UART_ALL_PERIPH(UARTx));
    assert_param(IS_UART_BAUDRATE(UART_InitStruct->UART_BitRate));
    ...
    divider = cpuclock / (UART_InitStruct->UART_BitRate >> 2);
    ...
    speederror = ((realspeed - UART_InitStruct->UART_BitRate) * 128)
            / UART_InitStruct->UART_BitRate;
	if (speederror > 2) {
...
void UART_StructInit(UART_InitTypeDef* UART_InitStruct)
{
  ...
  UART_InitStruct->UART_BitRate = 9600;
